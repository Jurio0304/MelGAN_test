...
def parse_args():
    ...
    parser.add_argument("--save_path", default='imagine_decode_result')
...
def main():
    ...
    if not os.listdir(logMel_root):
        ...
        s_t = np.load('log_mel/imagine_spectrogram.npy')
        ...
        voc = torch.empty(100, 1, 40, 423)
        for i in range(100):
            voc[i, :, :, :] = s_t[:, :, 423*i:423*i+1]
        ...
        with torch.no_grad():
            for i in range(100):
                ...
                pred_audio = pred_audio.squeeze().cpu()
                save_sample(root / ("imagine_generated_%d.wav" % i), 22050, pred_audio)
                print("imagine_generated_%d.wav stored" % i)
