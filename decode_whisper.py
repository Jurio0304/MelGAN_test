...
def parse_args():
    ...
    parser.add_argument("--save_path", default='whisper_decode_result')
    parser.add_argument("--load_path", default='test_result')
    parser.add_argument("--logMel_path", default='log_mel')
...
def main():
    ...
    logMel_root = Path(args.logMel_path) if args.logMel_path else None
    ...
    if not os.listdir(logMel_root):
        test_set = AudioDataset(
            "decode_test_file.txt",
            22050 * 4,
            sampling_rate=22050,
            augment=False,
        )
        ...
        s_t = np.load('log_mel/whisper_spectrogram.npy')
        s_t = np.transpose(s_t)
        s_t = s_t[:, 0:34500]
        # temp = np.zeros((40, 34400))
        # s_t = np.r_[s_t, temp]
        s_t = s_t[np.newaxis, :]
        ...
        voc = torch.empty(100, 1, 40, 345)
        for i in range(100):
            voc[i, :, :, :] = s_t[:, :, 345*i:345*i+1]
        ...
        with torch.no_grad():
            for i in range(100):
                voc_temp = voc[i, :, :, :].cuda()
                pred_audio = netG(voc_temp)
                pred_audio = pred_audio.squeeze().cpu()
                save_sample(root / ("whisper_generated_%d.wav" % i), 22050, pred_audio)
                print("whisper_generated_%d.wav stored" % i)
